...
extension RFC_3986.URI {
    ...
    public struct Userinfo: Sendable, Equatable, Hashable {
        ...
        private static func validate(_ string: String) throws {
            ...
            var allowedChars = RFC_3986.CharacterSets.userinfo
            allowedChars.insert(charactersIn: "%")

            // Check for any characters not in the allowed set
            if let invalidRange = string.rangeOfCharacter(from: allowedChars.inverted) {
                let invalidChar = string[invalidRange]
...
extension RFC_3986.URI.Userinfo: ExpressibleByStringLiteral {
    /// Creates a userinfo from a string literal without validation
    ///
    /// Example:
    /// ```swift
    /// let userinfo: RFC_3986.URI.Userinfo = "john"
    /// ```
    ///
    ...
    public init(stringLiteral value: String) {
        do {
            ...
            fatalError("Invalid userinfo literal: \(value) - \(error)")
        }
    }
}
